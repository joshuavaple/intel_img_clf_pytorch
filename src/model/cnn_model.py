...
class MyCnnModel(nn.Module):
    def __init__(self):
        ...
        self.fc14 = nn.Linear(64*28*28, 500) #output shape of maxpool3 is 64*28*28
        self.fc15 = nn.Linear(500, 50)
        self.fc16 = nn.Linear(50, 6) # output of the final DC layer = 6 = number of classes

    def forward(self, x):
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
        x = self.maxpool(x) # maxpool1 output shape is 16*112*112 (112 = (224-2)/2 + 1)
        x = F.relu(self.conv3(x))
        x = F.relu(self.conv4(x))
        x = self.maxpool(x) # maxpool2 output shape is 32*56*56 (56 = (112-2)/2 + 1)
        x = F.relu(self.conv5(x))
        x = F.relu(self.conv6(x))
        x = F.relu(self.conv7(x))
        x = self.maxpool(x)  # maxpool3 output shape is 64*28*28 (28 = (56-2)/2 + 1)
...
def train_model(model, device, train_loader, val_loader, criterion, optimizer, num_epochs=5):
    ...
    model = model.to(device)
    train_result_dict = {'epoch':[], 'train_loss':[], 'val_loss':[], 'accuracy':[], 'time':[]} 
